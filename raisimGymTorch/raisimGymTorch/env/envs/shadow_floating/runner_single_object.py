...
from raisimGymTorch.env.bin import shadow_floating as mano
from raisimGymTorch.env.RaisimGymVecEnvOther import RaisimGymVecEnvTest as VecEnv
from raisimGymTorch.helper.raisim_gym_helper import ConfigurationSaver, load_param
from raisimGymTorch.env.bin.shadow_floating import NormalSampler
...
exp_name = "shadow_floating_single"
...
saver = ConfigurationSaver(log_dir=exp_path + "/raisimGymTorch/" + args.storedir + "/" + task_name,
                           save_items=[task_path + "/cfgs/" + args.cfg, task_path + "/Environment.hpp",
                                       task_path + "/runner_single_object.py", task_path + "/../../RaisimGymVecEnvOther.py"], test_dir=test_dir)
...
if args.load_trained_policy:
    # Accept either an absolute weight path or the legacy relative default
    if os.path.isabs(weight_path):
        resolved_weight_path = weight_path
    else:
        resolved_weight_path = saver.data_dir.split('eval')[0] + weight_path

    load_param(resolved_weight_path, env, actor_r, critic_r, ppo_r.optimizer, saver.data_dir,
               cfg_grasp)
...
for update in range(args.num_iterations):
    ...
    # Evaluate trained model visually
    ...
    for step in range(n_steps_r):
        ...
        for i in range(num_envs):
            ...
            rewards_r[i]['reward_sum'] = (rewards_r[i]['reward_sum'] + rewards_r[i]['affordance_reward'] + rewards_r[i]['direction_reward'] + rewards_r[i]['center_reward']
