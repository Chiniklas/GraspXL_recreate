...
"""
Single-object version of partnet_eval.py.
Runs the shadow floating test env on only Mug_8556_handle from the mixed_train set.
"""
...
saver = ConfigurationSaver(log_dir=exp_path + "/raisimGymTorch/" + args.storedir + "/" + task_name,
                           save_items=[task_path + "/single_partnet_eval.py"], test_dir=test_dir)
...
for update in range(25):
    ...
    current_contact_ratio = np.sum(obs_r[:, 62:78], axis=1) / (np.sum(obs_r[:, 62:78], axis=1) + np.sum(obs_r[:, 100:116], axis=1) + 0.0000001)
...
file_name = "/aff_result_single_mug.txt"
