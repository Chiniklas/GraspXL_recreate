...
parser.add_argument('-itr', '--num_iterations', type=int, default=10001)
parser.add_argument('-re', '--load_trained_policy', action="store_true")
parser.add_argument('-ln', '--log_name', type=str, default='single_obj')
parser.add_argument('--cat', type=str, default='mixed_train',
                    help='object category folder under rsc/ to load (default: mixed_train)')
parser.add_argument('--object', type=str, required=True,
                    help='object folder name (inside the category) to train on')
parser.add_argument('--repeat', type=int, default=1,
                    help='duplicate the single object this many times to increase num_envs')
...
# Validate the requested object exists before proceeding
object_dir = os.path.join(directory_path, args.object)
if not os.path.isdir(object_dir):
    raise FileNotFoundError(f"Requested object '{args.object}' not found in {directory_path}")

obj_path_list = []
obj_list = [args.object for _ in range(max(1, args.repeat))]
